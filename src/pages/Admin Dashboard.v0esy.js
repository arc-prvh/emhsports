...
async function setAdminDashboardCounting() {
	...
	let parentCount = await wixData
		.query("Parents")
		.find()
		.then((res) => res.items.length);
	let studentCount = await wixData
		.query("Students")
		.find()
		.then((res) => res.items.length);
	let classCount = await wixData
		.query("Classes")
		.find()
		.then((res) => res.items.length);
	...
	$w("#totalStudent").text = `Total student count: ${studentCount}`;
	$w("#studentActive").text = `Total student active: ${await getActiveStudents()}`;
	$w("#studentInactive").text = `Total student inctive: ${await getInactiveStudents(studentCount)}`;
	$w("#studentPresent").text = `Total student inctive: ${await getPresentStudents()}`;
	$w("#studentAbsent").text = `Total student inctive: ${await getAbsentStudents()}`;
}
...
const getActiveStudents = async () => {
	let res = null;
	try {
		res = await wixData.query("Carts").eq("status", "Paid").distinct("student");
	} catch (error) {
		console.log(error);
	}
    return res.totalCount;
};
const getInactiveStudents = async (totalStudent) => {
    let res = null;
	try {
		res = await wixData.query("Carts").distinct("student");
	} catch (error) {
		console.log(error);
	}
    return (totalStudent - res.totalCount);
};
