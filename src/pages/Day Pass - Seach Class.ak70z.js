...
// --- Colour Constants ---
const textHeadingHtmlNormal = (text) =>
    `<p class="p1"><span>${text}</span></p>`;
const textHtmlNormal = (text) => `<p>${text}</p>`;

const textHeadingHtmlOnHoverAndSelected = (text) =>
    `<p class="p1" style='color:white'><span>${text}</span></p>`;
const textHtmlOnHoverAndSelected = (text) =>
...
$w.onReady(function () {
    ...
    $w('#classRepeater').onItemReady(($item, itemData, index) => {
        ...
        $item(
            '#timeSlot',
        ).text = `${itemData.timeslot[0].startTime} - ${itemData.timeslot[0].endTime}`;
    ...
    $w('#studentRepeater').onItemReady(($item, itemData, index) => {
        ...
        $item('#studentCard').onMouseIn((e) => {
            if (!itemData.isSelected) {
                $item('#studentNameHeading').html =
                    textHeadingHtmlOnHoverAndSelected('Name');
                $item('#studentGradeHeading').html =
                    textHeadingHtmlOnHoverAndSelected('Grade');
                $item('#studentGrade').html = textHtmlOnHoverAndSelected(
                    `${itemData.grade}`,
                );
                $item('#studentName').html = textHtmlOnHoverAndSelected(
                    `${itemData.name}`,
                );

                $item('#studentCard').style.backgroundColor =
                    studentCardMouseInColour;
            }
        });
        $item('#studentCard').onMouseOut((e) => {
            if (!itemData.isSelected) {
                $item('#studentNameHeading').html =
                    textHeadingHtmlNormal('Name');
                $item('#studentGradeHeading').html =
                    textHeadingHtmlNormal('Grade');
                $item('#studentGrade').html = textHtmlNormal(
                    `${itemData.grade}`,
                );
                $item('#studentName').html = textHtmlNormal(`${itemData.name}`);

                $item('#studentCard').style.backgroundColor =
                    studentCardMouseOutColour;
            }
        });
    });
    ...
    $w('#addressRepeater').onItemReady(($item, itemData, index) => {
        ...
        $item('#addressCard').onMouseIn((e) => {
            if (!itemData.isSelected) {
                $item('#parkAddress').html = textHtmlNormal(
                    `${itemData.parkAddress}`,
                );
                $item('#addressCard').style.backgroundColor =
                    studentCardMouseInColour;
            }
        });
    ...
    $w('#selectedMonth').onItemReady(($item, itemData, index) => {
        $item('#month').text = `${getMonthName(itemData.month)} (${
            itemData.startTime
        }-${itemData.endTime})`;
    ...
    $w('#virtualSelectedMonth').onItemReady(($item, itemData, index) => {
        $item('#virtualMonth').text = `${getMonthName(itemData.month)} (${
            itemData.startTime
        }-${itemData.endTime})`;
    ...
    $w('#timeSlotRepeater').onItemReady(($item, itemData, index) => {
        $item(
            '#availableTimeSlot',
        ).text = `${itemData.startTime} - ${itemData.endTime}`;
...
function fillMoreInfoData(classData) {
    ...
    $w('#monthDropDown').onChange(() => {
        ...
        $w('#timeslotPE').text = getTimeSlotInString(
            $w('#monthDropDown').value,
        );
    });
...
function fillVirtualMoreInfoData(classData) {
    ...
    $w('#timeslotVirtual').text = getTimeSlotInString(
        $w('#virtualMonthDropDown').value,
    );
...
async function renderStudentsList() {
    ...
    if (!currentUserWixid)
        console.log('Some error occured in recognising logged in user');
    else {
        await wixData
            .query('Students')
            ...
            .then((res) => {
                if (res.length == 0) {
                    $w('#noStudentProfileErrorMessage').text =
                        noStudentFoundMessage;
                    $w('#noStudentProfileErrorMessage').show();
                } else {
                    let studentRepeaterData =
                        formatResponseForDefaultStudentRepeater(res);
...
const datePickerChangeHandler = () => {
    ...
    selectedClass.timeslot.forEach((timeSlot) => {
        if (
            parseInt(timeSlot.month) - 1 == month &&
            parseInt(timeSlot.day) == day
        ) {
...
const isAlreadyInWishlist = async (classId) => {
    ...
    let currentUserWixid = await setCurrentUserWixId();
    wixData
        .isReferenced('Classes', 'wishlist', classId, currentUserWixid)
        .then((res) => {
            if (!res) {
...
function checkIfClassInCartOrUnderReview(classData) {
    if (classData.status == inCart || classData.status == underReview)
        return true;
    return false;
}
...
function checkItemOverlaps(array1, array2) {
    ...
    for (const item1 of array1) {
        for (const item2 of array2) {
            if (item1.monthValue == item2.monthValue)
                overlappingItems.push(item2.monthLabel);
        }
    }
...
async function validateItemsToBeAddedInCart(items) {
    for (const item of items) {
        ...
        for (const cartItem of cartItems.items) {
            let overlappingMonths = checkItemOverlaps(
                cartItem.selectedMonths,
                selectedMonth,
            );
    ...
    for (let i = 0; i < renderedStudentsData.length; i++) {
        if (renderedStudentsData[i]['_id'] == itemId) {
            if (renderedStudentsData[i]['isSelected']) {
                ...
                renderedStudentsData[i]['bgColour'] =
                    studentCardNotSelectedColour;

                renderedStudentsData[i]['nameHeadingHtml'] =
                    textHeadingHtmlNormal('Name');
                renderedStudentsData[i]['gradeHeadingHtml'] =
                    textHeadingHtmlNormal('Grade');
                renderedStudentsData[i]['nameValueHtml'] = textHtmlNormal(
                    renderedStudentsData[i].name,
                );
                renderedStudentsData[i]['gradeValueHtml'] = textHtmlNormal(
                    renderedStudentsData[i].gradeLevel,
                );
            } else {
                ...
                renderedStudentsData[i]['gradeHeadingHtml'] =
                    textHeadingHtmlOnHoverAndSelected('Grade');
                renderedStudentsData[i]['nameValueHtml'] =
                    textHtmlOnHoverAndSelected(renderedStudentsData[i].name);
                renderedStudentsData[i]['gradeValueHtml'] =
                    textHtmlOnHoverAndSelected(
                        renderedStudentsData[i].gradeLevel,
                    );
            }
        }
...
export async function nextButtonInSelectStudentState_click(event) {
    let selectedStudents = renderedStudentsData.filter(
    ...
    for (let i = 0; i < selectedStudents.length; i++)
        selectedStudents[i]['formFilled'] = false;
...
const renderAvailableDates = () => {
    ...
    timeSlots.forEach((timeSlot) => {
        const stringDates = getAllDaysOfTimeSlot(
            timeSlot.month,
