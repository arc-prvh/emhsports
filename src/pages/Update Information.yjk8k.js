...
export function updateCoachButton_click(event) {
    ...
    if ($w('#updateCity').value == "" || $w('#updateZipCode').value == "" || $w('#updateAddress').value == "" ||
        $w('#updatePhone').value == "" || $w('#updateState').value == "" || $w('#repeaterSport').data == []) {
        ...
        wixData.get('Coach', memberId)
            .then(async (toCoachUpdate) => {
                toCoachUpdate.city = $w('#updateCity').value,
                    ...
                    await wixData.update("Coach", toCoachUpdate)
                    .then((results) => {
                        console.log(results);
...
$w('#studentRepeater').onItemReady(($item, itemData, i) => {
    ...
    $item('#openButton').onClick(() => { selectItemForUpdate(itemData) })
...
const selectItemForUpdate = (itemData) => {
    $w('#studentBox').expand();
    $w('#studentNameText').text = itemData.name,
        $w('#studentAddress').value = itemData.address;
    ...
    wixData.query('Students').eq('_id', itemData._id).find().then((res) => {
...
export function studentUpdateButton_click(event) {
    ...
    //     }
    // } else {
    wixData.get('Students', currentStudent)
        .then((toUpdateStudent) => {
            ...
            wixData.update("Students", toUpdateStudent)
                .then((results) => {
...
function addNewSport(currentSport) { //ADDING FUCTION TO ADD SPORT IN REPEATER
...
function deletesport(id) { //DELETESPORT FROM REPEATER
...
function addNewAllergies(currentAllergies) { //ADDING ALLERGY IN REPEATER
    const includesAllergies = (currentAllergies) => {
        for (const all_ergy of allergy)
            if (all_ergy.allergy.toLowerCase() == currentAllergies.toLowerCase()) return true
        // 
        return false
    }
    let allergyToInsert = {
...
function deleteAllergy(id) { //DELETE ALLERGY FROM REPEATER
