...
$w.onReady(function () {
    ...
    $w('#scheduleRepeater').onItemReady(($item, itemData, index) => {
        $item("#classSchedule").text = itemData.date;
    })
    ...
    $w('#selectedMonth').onItemReady(($item, itemData, index) => {
        $item('#month').text = itemData.monthLabel;
        $item('#removeMonth').onClick(() => { removeMonthHandler(itemData._id) });
    })
    $w('#virtualSelectedMonth').onItemReady(($item, itemData, index) => {
        $item('#virtualMonth').text = itemData.monthLabel;
        $item('#virtualRemoveMonth').onClick(() => { virtualRemoveMonthHandler(itemData._id) });
    })

    wixData.query('Classes').eq('status', 'Active').eq('classType', 'PE').find().then(res => {
    ...
    // City Dropdown Set
    wixData.query('Classes').find().then(res => {
        const cityOption = [{
            label: 'Clear',
            value: 'Clear'
        }]
    ...
    $w('#addMonth').onClick(addMonthHandler);
...
function fillMoreInfoData(classData) {
    ...
    // $w('#classSlot').value = formatTimeSlot(classData.day, classData.startTime, classData.endTime);
    // $w('#timeslotDropdown').options = getTimeSlotForDropdownData(classData.timeslot);
    $w('#timeslotPE').text = `${classData.timeslot[0].startTime} - ${classData.timeslot[0].endTime}`
    $w('#scheduleRepeater').data = getAllDaysOfTimeSlot($w('#monthDropDown').value, classData.timeslot);
    $w('#monthDropDown').onChange(() => {
        $w('#noMonthSelectedMsg').collapse();
        $w('#scheduleRepeater').data = getAllDaysOfTimeSlot($w('#monthDropDown').value, classData.timeslot);
        $w('#sportName').text = getSportName($w('#monthDropDown').value);
        classData.timeslot.forEach(el => {
            if (el.month === $w('#monthDropDown').value) {
                $w('#timeslotPE').text = `${el.startTime} - ${el.endTime}`
            }
        })
    })
    $w('#scheduleRepeater').expand();
    ...
    // const economy = classData.package['Economy'];
    // const deluxe = classData.package['Deluxe'];
    // const premium = classData.package['Premium'];

    // Don't try to change this from database as array, or else you have to change from other places too.
    // If you have time, I recommend to optimise it or else leave it as it is
    // const packageData = [{
    //     _id: 'Economy-1',
    //     name: "Economy",
    //     cost: economy.cost.toString(),
    //     description: economy.description,
    // },
    // {
    //     _id: 'Deluxe-2',
    //     name: 'Deluxe',
    //     cost: deluxe.cost.toString(),
    //     description: deluxe.description,
    // },
    // {
    //     _id: 'Premium-3',
    //     name: 'Premium',
    //     cost: premium.cost.toString(),
...
function fillVirtualMoreInfoData(classData) {
    ...
    $w('#timeslotVirtual').text = `${classData.timeslot[0].startTime} - ${classData.timeslot[0].endTime}`
    $w('#virtualScheduleRepeater').data = getAllDaysOfTimeSlot($w('#virtualMonthDropDown').value, classData.timeslot);
    $w('#virtualMonthDropDown').onChange(() => {
        $w('#noVirtualMonthSelectedMsg').collapse();
        $w('#virtualScheduleRepeater').data = getAllDaysOfTimeSlot($w('#virtualMonthDropDown').value, classData.timeslot);
        classData.timeslot.forEach(el => {
            if (el.month === $w('#virtualMonthDropDown').value) {
                $w('#timeslotVirtual').text = `${el.startTime} - ${el.endTime}`
            }
        })
    })
    ...
    $w('#virtualScheduleRepeater').expand();
        ...
        .then(async member => {
            await wixData.query('Parents')
                ...
                .then(parent => {
                    ...
                    console.log('currentUserWixid', currentUserWixid)
                }).catch(err => {
...
const getAllDaysOfTimeSlot = (currentMonthText, timeSlot) => {
    ...
    const daysArray = [...getAllDays(month, day)]
    return daysArray.slice(0, 4);
...
export const getAllDays = (currentMonthText, daytext) => {
    ...
    while (d.getMonth() === month) {
        dates.push({ _id: Math.floor(Math.random() * 100000).toString(), date: new Date(d.getTime()).toDateString() });
...
const addMonthHandler = () => {
    ...
    const monthOptions = $w('#monthDropDown').options;
    const selectedIndex = $w('#monthDropDown').selectedIndex;
    const monthLabel = monthOptions[selectedIndex].label;
    const selectedMonthData = $w('#selectedMonth').data;
    selectedMonthData.push({ _id: monthValue, monthValue, monthLabel });
...
const virtualAddMonthHandler = () => {
    $w('#noVirtualMonthSelectedMsg').collapse();
    const monthValue = $w('#virtualMonthDropDown').value;
    const monthOptions = $w('#virtualMonthDropDown').options;
    const selectedIndex = $w('#virtualMonthDropDown').selectedIndex;
    const monthLabel = monthOptions[selectedIndex].label;
...
export const addToWishlistHandler = async (classId) => {
    let currentUserWixid = await setCurrentUserWixId();
    wixData.insertReference('Classes', 'wishlist', classId, currentUserWixid)
        .then(() => {
...
async function validateItemsToBeAddedInCart(items) {
    ...
    for (const item of items) {
        ...
        const apForm = await wixData.query('AthleticParticipationForm')
            .eq('student', studentId)
            .eq('status', active)
            .include('student')
            .find().then(res => res.items[0])
        ...
        const cartItems = await wixData.query('Carts')
            .eq('student', studentId)
            .eq('class', classId)
            .include('class')
            .include('student')
        ...
        for (const cartItem of cartItems.items) {
            ...
            if (overlappingMonths.length > 0) {
                return {
                    showError: true,
                    message: `${cartItem.student.name} registration in ${selectedClass.parkName} is overlapping with existing months. Change the months and try again.
...
export async function search_click(event) {
    ...
    await classQuery.find().then(res => {
...
export async function nextButtonInSelectStudentState_click(event) {
    ...
    if (!validationResult.showError) {
        wixData.bulkInsert('Carts', toInsertInCart).then(res => {
